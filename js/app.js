...
var GameUI = {
  ...
  displayChoices: function() {
    ...
    for (var i = 0; i < choices.length; i++) {
      html += `<div class="col-sm-6"><button type="button" class="raise" id="choice-${i}">${choices[
        i
      ]}</button></div>`;
    }
  ...
  displayHelpCall: function() {
    swal({
      title:
        "<img src='http://images.techtimes.com/data/images/full/239267/facebook-ceo-mark-zuckerberg.jpg' width='80%' class='text-center'/>",
      text:
        "Tôi chắc chắn 100% là đáp án " + "<h2>" + game.helpCall() + "</h2>",
      html: true
    });
  ...
  countDown: function() {
    ...
    $("#clock")
      .countdown(tenSeconds, function(event) {
        $(this).text(event.strftime("%S"));
      })
      .on("finish.countdown", function(event) {
        GameUI.displayGameOver(
          "Hết Giờ ! Bạn đã ra về với " + game.getScore() + " điểm."
        );
      });
  },
      ...
      if (game.guess(answer)) {
        ...
        setTimeout(() => {
          this.displayGameOver(
            "Bạn đã ra về với " + game.getScore() + " điểm !"
          );
        }, 3000);
      }
    }, 1000);
